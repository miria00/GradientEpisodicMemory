...
class Net(torch.nn.Module):
    ...
    def forward(self, x, t):
        ...
        for cc in range(offset1, offset2):
            means[cc - offset1] = self.net(self.mem_class_x[cc]).data.mean(0)
    ...
    def observe(self, x, t, y):
        ...
        if self.examples_seen == self.samples_per_task:
            ...
            for ll in range(num_classes):
                ...
                mean_feature = self.net(cdata)[:, offset1:offset2].data.clone().mean(0)
                nd = self.nc_per_task
                ...
                for ee in range(self.num_exemplars):
                    ...
                    if ee > 0:
                        prev = (
                            self.net(exemplars[:ee])[:, offset1:offset2]
                            .data.clone()
                            .sum(0)
                        )
                    cost = (
