...
class Net(torch.nn.Module):
    def __init__(self, n_inputs, n_outputs, n_tasks, args):
        ...
        for _ in range(n_tasks):
            if self.is_cifar:
                self.nets.append(ResNet18(int(n_outputs / n_tasks), int(20 / n_tasks)))
            else:
                self.nets.append(
        ...
        for t in range(n_tasks):
            self.opts.append(torch.optim.SGD(self.nets[t].parameters(), lr=args.lr))
    ...
    def forward(self, x, t):
        ...
        if self.is_cifar:
            ...
            bigoutput[
                :, int(t * self.nc_per_task) : int((t + 1) * self.nc_per_task)
            ].copy_(output.data)
            return bigoutput
        else:
            return output
    ...
    def observe(self, x, t, y):
        ...
        if self.finetune and t > 0 and t != self.old_task:
            ...
            for ppold, ppnew in zip(
                self.nets[self.old_task].parameters(), self.nets[t].parameters()
            ):
