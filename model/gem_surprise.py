...
class Net(nn.Module):
    def __init__(self, n_inputs, n_outputs, n_tasks, args):
        ...
        self.memory_data = torch.zeros(
            n_tasks, self.n_memories, n_inputs, dtype=torch.float
        )
    ...
    def observe(self, x, t, y):
        ...
        # update memory buffer with least correct examples from new minibatch
        # and current memory
        mem_x = self.memory_data[t]
        mem_y = self.memory_labs[t]
        cands_x = torch.cat([mem_x, x], dim=0)
        cands_y = torch.cat([mem_y, y], dim=0)
        with torch.no_grad():
            out = self.forward(cands_x, t)
            preds = out.gather(dim=1, index=cands_y[:, None])
            preds = preds.squeeze()
            mink = torch.topk(preds, k=self.n_memories, dim=0, largest=False)
            self.memory_data[t] = cands_x[mink.indices]
            self.memory_labs[t] = cands_y[mink.indices]
